class Example extends Phaser.Scene {
    constructor() {
        ...
        const baseBuffer = audioContext.createBuffer(1, audioContext.sampleRate * 0.1, audioContext.sampleRate);
        const baseData = baseBuffer.getChannelData(0);
        for (let i = 0; i < baseBuffer.length; i++) {
            baseData[i] = Math.sin(330 * Math.PI * 2 * i / audioContext.sampleRate) *
                Math.exp(-4 * i / baseBuffer.length);
        }
        // Paddle hit sound (higher pitch)
        const paddleBuffer = audioContext.createBuffer(1, audioContext.sampleRate * 0.05, audioContext.sampleRate);
        const paddleData = paddleBuffer.getChannelData(0);
        for (let i = 0; i < paddleBuffer.length; i++) {
            paddleData[i] = Math.sin(660 * Math.PI * 2 * i / audioContext.sampleRate) *
    ...
    preload() {
        this.load.image('background', 'https://play.rosebud.ai/assets/night_02.png?lmty');

        // Setup socket event listeners if Socket.IO is available
        if (typeof io !== 'undefined') {
            ...
            this.playerNumber = 1; // Default to player 1 in local mode
        }
    }
    ...
    create() {
        ...
        const createSwitch = (isLeft) => {
            ...
            if (isLeft) {
                this.goalLeft.add([switchComponents.base, switchComponents.glow, switchComponents.lever]);
                this.leftLever = switchComponents.lever;
            } else {
                this.goalRight.add([switchComponents.base, switchComponents.glow, switchComponents.lever]);
        ...
        // Add base labels
        this.add.text(175, 575, 'Oncogene', {
            fontSize: '20px',
            fill: '#00ffff',
            align: 'center',
            fontFamily: 'Arial'
        }).setOrigin(0.5);
        this.add.text(625, 575, 'Tumor\nsuppressor gene', {
            fontSize: '20px',
            fill: '#ff3366',
            align: 'center',
            fontFamily: 'Arial'
        ...
        this.cancerTextRight = this.add.text(625, 35, 'CANCER', {
            fontSize: '32px',
            fill: '#ff0000',
            align: 'center',
            fontFamily: 'Arial',
        ...
        this.cancerTextLeft = this.add.text(175, 35, 'CANCER', {
            fontSize: '32px',
            fill: '#ff0000',
            align: 'center',
            fontFamily: 'Arial',
        ...
        const createTexture = (name, color, size, radius) => {
            ...
            if (name === 'puck') {
                ...
                graphics.beginPath();
                graphics.arc(shadowSize / 2 + 4, shadowSize / 2 + 4, radius + 4, 0, Math.PI * 2);
                graphics.closePath();
                graphics.fillPath();
                ...
                graphics.beginPath();
                graphics.arc(shadowSize / 2 + 2, shadowSize / 2 + 2, radius + 2, 0, Math.PI * 2);
                graphics.closePath();
                graphics.fillPath();
            } else {
                ...
                graphics.fillStyle(0x000000, 0.3);
                graphics.beginPath();
                graphics.arc(shadowSize / 2, shadowSize / 2, radius + 2, 0, Math.PI * 2);
                graphics.closePath();
                graphics.fillPath();
            }
            ...
            graphics.beginPath();
            graphics.arc(size / 2 - radius / 3, size / 2 - radius / 3, radius / 2, 0, Math.PI * 2);
            graphics.closePath();
            graphics.fillPath();
            // Add secondary highlight for more depth
            graphics.fillStyle(highlightColor, 0.2);
            graphics.beginPath();
            graphics.arc(size / 2 - radius / 4, size / 2 - radius / 4, radius / 3, 0, Math.PI * 2);
            graphics.closePath();
            graphics.fillPath();
        ...
        this.paddle1.body.setBoundsRectangle(new Phaser.Geom.Rectangle(75, 75, 200, 450));
        this.paddle2.body.setBoundsRectangle(new Phaser.Geom.Rectangle(525, 75, 200, 450));
        ...
        const initializeGameObject = (obj, config) => {
            obj.setBounce(config.bounce || 0)
                .setCollideWorldBounds(true)
                .setImmovable(config.immovable || false);

            if (config.circle) {
                obj.body.setCircle(config.radius, config.offsetX || 0, config.offsetY || 0);
            }

            if (config.bounds) {
                obj.body.setBoundsRectangle(new Phaser.Geom.Rectangle(...config.bounds));
            }
        };
        ...
        this.physics.add.collider(this.puck, [this.paddle1, this.paddle2], this.handlePaddleCollision, null, this);
        // Enable mouse input
        this.input.on('pointermove', (pointer) => {
            if (!this.isPlayerReady) return;
            let x, y;
            const lerpFactor = 0.5; // Adjust this value between 0 and 1 for smoother movement
            if (this.playerNumber === 1) {
            ...
            // Emit position to server if socket is available
            if (x && y && this.socket) {
    ...
    handlePaddleCollision(puck, paddle) {
        ...
        const puckVel = new Phaser.Math.Vector2(puck.body.velocity.x, puck.body.velocity.y);
        const paddleVel = new Phaser.Math.Vector2(paddle.body.velocity.x, paddle.body.velocity.y);
